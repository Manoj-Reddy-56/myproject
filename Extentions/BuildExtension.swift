...
//  Pocofy
//
//  Created by Raja Bhuma on 21/11/17.
...
extension UIScreen {
    ...
    var screenSize_inch: String {
        
        let scaleScreen:Double = Double(scale)
        let ppi = scaleScreen * (UI_USER_INTERFACE_IDIOM() == .pad ? 132.0 : 163.0)
        
        let width = Double(UIScreen.main.bounds.size.width) * scaleScreen
        let height = Double(UIScreen.main.bounds.size.height) * scaleScreen
        
        let horizontal = width / ppi, vertical = height / ppi
        
        let diagonal = sqrt(pow(horizontal, 2) + pow(vertical, 2))
        
        return String.init(format: "%.2f", diagonal)
    }
}
...
public enum DeviceType: String, EnumProtocol {
    ...
    internal var identifiers: [String] {
        ...
        case .iPadPro10_5Inch: return ["iPad7,3", "iPad7,4"]
        case .iPadPro12Inch: return ["iPad6,7", "iPad6,8", "iPad7,7", "iPad7,8"]
        }
    }
